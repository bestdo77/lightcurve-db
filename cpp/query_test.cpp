...
public:
    TDengineQueryTester(const std::string& host = "localhost",
                       ...
                       int port = 6030,
                       const std::string& database = "sensor_db_healpix",
    ...
    bool loadTestData(const std::string& csv_file) {
        ...
        std::cout << "✅ 成功读取数据，唯一source_id数量：" << unique_sources.size() << std::endl;
...
int main() {
    try {
        std::cout << "🌟 TDengine HealPix 空间索引性能测试器 (C++ 版本)" << std::endl;
        std::cout << "============================================================" << std::endl;
        
        TDengineQueryTester tester;
        
        // 加载测试数据
        if (!tester.loadTestData("data/test_data.csv")) {
            std::cerr << "请确认数据文件路径正确，或使用数据生成器生成测试数据" << std::endl;
            return 1;
        }
